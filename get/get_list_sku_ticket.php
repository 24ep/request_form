...
//   </div>';
    ...
    echo 
    "<p>
    <a href='#' id='download_link' style='text-decoration: none' class='badge bg-dark bg-gradient shadow-sm mt-2 p-1 pe-3 me-2 ms-2' onClick='javascript:ExcelReport();''>
    <ion-icon name='download-outline'></ion-icon>Export</a>
    ";
    
       echo '<button type="button" style="margin-top:10px"
          onclick="force_sync_with_ticket('.$id.',&#34;'.$bu.'&#34;)"
          class="badge bg-danger bg-gradient shadow-sm mt-2 p-1 pe-3 me-2">ยืนยัน เชื่อมต่อ ด้านล่างกับ ticket</button>';
          
    echo "
    <textarea style='font-size:12px' oninput='sku_checking()' class='form-control mt-2'
          id='sku_checking' name='sku_accepted'
          placeholder='ตรวจสอบ IBC ตามตัวอย่างด้านล่าง วางตามตัวอย่างด้านล่าง&#10;&#10;3466644&#10;2443356&4735'
          rows='10' style='height: 20px'></textarea>
    
    </p>";

  echo '<div id="sku_checking_result"></div>
       <div id="sku_checking_result_force"></div>
   <input type="hidden" id="result_checking_sku" name="result_checking_sku"
         value="">
   ';
