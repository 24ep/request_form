...
<?php $i++; }
        ...
        list_ts_non_status("(".$filter.") and ( lower(ticket.participant) like  lower('%".$_SESSION["ts_username"]."%') or  lower(ticket.case_officer) like lower('%".$_SESSION["ts_username"]."%') )   and ticket.status = '".$row_status["attribute_option"]."'",$ts_command_limit  );
        echo '</div>';
        $i++;
        }
