...
$start_item =  intval(intval($_POST['page_navigator_input']) -1 )* 30;
$limit="LIMIT 30";
...
//count item
$con= mysqli_connect("service-gate-cds-omni-service-gate.a.aivencloud.com","avnadmin","AVNS_lAORtpjxYyc9Pvhm5O4","all_in_one_project","10628") or die("Error: " . mysqli_error($con));
$query = "SELECT count(*) as count_item FROM all_in_one_project.add_new_job as anj where (".$filter.")"
...
//get list
$con= mysqli_connect("service-gate-cds-omni-service-gate.a.aivencloud.com","avnadmin","AVNS_lAORtpjxYyc9Pvhm5O4","all_in_one_project","10628") or die("Error: " . mysqli_error($con));
$query = "SELECT * FROM all_in_one_project.add_new_job as anj where (((".$filter.") ".$parent_filter." )
...
while($row = mysqli_fetch_array($result)) {
    ...
    if($row["config_type"]=="parent"){
        ...
        $task_status = '';
        $query_sum="SELECT sum(sku) as total from all_in_one_project.add_new_job where parent = ".$row["id"];
...
if($_SESSION['display_ticket_detail']!='popup'){
    ...
    $ticket .= '<button type="button" class="badge rounded bg-gradient bg-dark p-2 ps-3 pe-3"
    data-bs-toggle="modal" data-bs-target="#exampleModal"
    onclick="call_model_edit_add_new('.$row["id"].')"
    name="open-outline" role="img" aria-label="open outline">Detail
    </button>';
}
    $ticket .= "</div>";
    $ticket .=  "</li>";
    //get sub ticket
    $con= mysqli_connect("service-gate-cds-omni-service-gate.a.aivencloud.com","avnadmin","AVNS_lAORtpjxYyc9Pvhm5O4","all_in_one_project","10628") or die("Error: " . mysqli_error($con));
    $query_count="SELECT count(*) as total from all_in_one_project.add_new_job where parent = ".$row["id"];
    $result_count = mysqli_query($con, $query_count);
    $data_count=mysqli_fetch_assoc($result_count);
    $subtask_count = $data_count['total'];
    if(isset($subtask_count) and $subtask_count <> 0 and $subtask_count <>null){
        $query_child = "SELECT * FROM all_in_one_project.add_new_job where  parent = ".$row["id"]." order by id ASC"  or die("Error:" . mysqli_error($con));
        date_default_timezone_set("Asia/Bangkok");
        mysqli_query($con, "SET NAMES 'utf8' ");
