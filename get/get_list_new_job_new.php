 ...
 if(isset($_SESSION['fopenticket'])){
  ...
  while($row = mysqli_fetch_array($result)) {
    ...
    if($row["config_type"]=="parent"){
      //set style
      $tr_class = "class='row sub-ticket shadow-sm p-2 mb-0 rounded ".$style_cancel."' style='background: white;align-items: center;'";
      $task_status = '';
      ...
      $tr_class = "class='row shadow-sm p-2 mb-2 rounded ".$style_cancel."' style='background: white;align-items: center;'";
      $task_status = $status ;
      $subtask_sum = $row["sku"];
    }
