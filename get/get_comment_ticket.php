    ...
    while($row = mysqli_fetch_array($result)) {
        ...
        while($row_attach = mysqli_fetch_array($result_attach)) {
            if($row_attach['is_image']==1){
                ...
                $list_file .= '<a type="button" href="'.$row_attach['file_path'].$row_attach['file_name'].'" style="margin-right:0px;margin-bottom:5px;font-size: 12px;border-color: #7ec1a2;"class="btn btn-outline-success btn-sm "><ion-icon name="document-outline" style="font-size: 12px;"></ion-icon>'.$row_attach['file_name'].'</a>';
            }
