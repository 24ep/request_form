...
function get_attachment_cr($id){
  ...
  <ul class="list-group ">';
    while($row = mysqli_fetch_array($result)) {
      ...
      <a href="'.$row['file_path'].$row['file_name'].'" download="'.$row['file_name'].'"><ion-icon name="cloud-download-outline" style="color:blue"></ion-icon></a>
      </li>';
      $pass = true;
    }
...
function get_image_cr($id){
  ...
  $list_image.= '<div class="row">';
    while($row = mysqli_fetch_array($result)) {
      ...
      <figure class="figure">
      <img src="'.$row["file_path"].$row['file_name'].'" class="img-thumbnail img-fluid" alt="'.$row["file_name"].'  " style="object-fit:cover;width:180px;height:180px;">
