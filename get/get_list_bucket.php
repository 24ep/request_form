...
//     ';
  while($row = mysqli_fetch_array($result)) {
    if(strpos( $_SESSION["prefix_project_sticky"],$row['prefix'])!==false){
        ...
        <label onclick="update_project_sticky_badge();run_ts_command(&#39task&#39);"  class="btn btn-outline-primary btn-sm bk-cr shadow-sm" for="'.$row["prefix"].'" >'.$row["project_name"].'</label>
    ';
    }else{
        echo '
