...
function get_comment_ms($id){
    date_default_timezone_set("Asia/Bangkok");
    $con= mysqli_connect("localhost","cdse_admin","@aA417528639","all_in_one_project") or die("Error: " . mysqli_error($con));
    mysqli_query($con, "SET NAMES 'utf8' ");
    $query = "SELECT * FROM comment WHERE ticket_type = 'message_box' and ticket_id = ".$id." ORDER BY id ASC" or die("Error:" . mysqli_error($con));
     $result = mysqli_query($con, $query);
      while($row = mysqli_fetch_array($result)) {
        $query_attach = "SELECT * FROM attachment WHERE ticket_type = 'message_box' and ticket_id = ".$row['id']." ORDER BY id ASC" or die("Error:" . mysqli_error($con));
        ...
        while($row_attach = mysqli_fetch_array($result_attach)) {
            ...
            if($row_attach['is_image']==1){
                $list_image .= '<div class="baguetteBoxFour gallery"><a target= "_blank" href="'.$herf.'"><img src="'.$herf.'"  class="img-thumbnail" alt="'.$row_attach['file_name'].'" style="object-fit:cover;width:180px;height:180px;margin-right:5px"></a></div>';
            }else{
