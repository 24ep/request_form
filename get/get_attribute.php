...
function get_username($att_code,$att_name,$site_element,$current_value,$code_element,$enable_edit,$id,$prefix,$database,$table,$primary_key_id,$require){
  ...
  while($option = mysqli_fetch_array($result_op)) {
    ...
    $option_element .= "<option ".$selected ." value='".$option["username"]."'>".$option['nickname'].' - '.$option['firstname'].' '.$option['lastname']." </option>";
  }
  if($enable_edit==''){
    // $badge_edit_lv = '<ion-icon style="color:#707684;margin-left:3px" name="color-wand-outline"></ion-icon>';
    $badge_edit_lv  ='';
...
function update_value_attribute(id, attribute_code, prefix, database, table, primary_key_id) {
    ...
    if (id) {
        $.post("/action/action_update_value_attribute.php", {
                id: id,
