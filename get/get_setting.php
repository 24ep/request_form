<?php
 ...
 while($row = mysqli_fetch_array($result)) {
    include("get_page_".$row["code"].".php");
    $function_account = get_page_account();
    ...
    $detail.= '<div class="tab-pane fade" id="v-pills-setting_'.$row["code"].'" role="tabpanel" aria-labelledby="v-pills-setting_'.$row["code"].'-tab"><h3>'.$row["value"].'</h3>';
         if($row["code"]=="account"){
            $detail.= $function_account;
         }
    $detail.='</div>';
}
