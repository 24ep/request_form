...
function get_origin_of_ticket($col,$table,$select_option,$sorm,$database) {
    ...
    while($row = mysqli_fetch_array($result)) {
        if($select_option==$row[$col]){
            ...
            data-html="<div class=&#39;origin_block_in&#39;><strong>'.$row['origin'].'</strong><br>'.$row['description'].'<br><a onclick="table_detail_page('.$row['id'].',&#39;'.$row['system_related'].'&#39;,&#39;origin_of_ticket&#39;,&#39;update&#39;)">SEE MORE</a></div>"
            >'.$row[$col].'</option>';
        }else{
