...
function get_origin_of_ticket($col,$table,$select_option,$sorm,$database) {
    ...
    while($row = mysqli_fetch_array($result)) {
        if($select_option==$row[$col]){
            ...
            data-html="<div class=&#39;origin_block&#39;><strong>'.$row['origin'].'</strong><br>'.$row['description'].'</div>"
            >'.$row[$col].'</option>';
        }else{
            $option_set .= '<option value="'.$row[$col].'"
            data-html="<div class=&#39;origin_block&#39;><strong>'.$row['origin'].'</strong><br>'.$row['description'].'</div>">'.$row[$col].'</option>';
        }
