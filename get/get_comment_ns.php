...
function get_comment_ns($id){
    ...
    $query = "SELECT * FROM comment WHERE ticket_type = 'add_new' and ticket_id = ".$id." ORDER BY id ASC" or die("Error:" . mysqli_error($con));
     $result = mysqli_query($con, $query);
      while($row = mysqli_fetch_array($result)) {
        $query_attach = "SELECT * FROM attachment WHERE ticket_type = 'add_new' and ticket_id = ".$row['id']." ORDER BY id ASC" or die("Error:" . mysqli_error($con));
    ...
    '.$image_profile.'
          <div class="fw-bold ms-2 me-2">'.$row['comment_by'].'</div>
