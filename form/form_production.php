...
function get_attribute($attribute_set,$section_group,$table,$database,$primary_key_id,$prefix_table){
    ...
    $query = "SELECT  * FROM ".$table.".job_attribute 
...
$query = "SELECT distinct attribute_set FROM ".$database.".job_attribute where allow_display=1 and  table='".$table."' order by sort_attribute_set" or die("Error:" . mysqli_error($con));
  $result = mysqli_query($con, $query);
  $d_attribute_set="";
  $d_attribute_section="";
  while($row = mysqli_fetch_array($result)) {
