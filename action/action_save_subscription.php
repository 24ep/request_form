...
if(is_array($data) && isset($data['endpoint'])){
  ...
  if($selectId->num_rows == 0 && isset($_GET['subscribe'])){
    ...
    $query = $con->query("INSERT INTO `push_subscribers` (`endpoint`, `expirationTime`, `p256dh`, `authKey`,`username`) VALUES ('{$data['endpoint']}', '{$data['expirationTime']}', '{$data['keys']['p256dh']}', '{$data['keys']['auth']}')");
