...
function sendline($id,$value_name,$value_change,$prefix){
        if($prefix=='ANJ' or $prefix=='JC' or $prefix=='NS' or $prefix=='RJ' ){
                $callback_url  = $_SERVER['DOCUMENT_ROOT'].'/?env=poojaroonwit&page=create_new&prefix=NS&direct='.$id;
                $prefix_post = 'NS';
        }
        if($prefix=='CR'){
            $callback_url  = $_SERVER['DOCUMENT_ROOT'].'/?env=poojaroonwit&page=update_content&prefix=CR&direct='.$id;
            $prefix_post = 'CR';
    }
