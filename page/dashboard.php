...
function get_list_status($username,$status){
    ...
    while($row = mysqli_fetch_array($result)) {
        $list .= '<li onclick="call_edit_add_new_modal('.$row["id"].')" class="list-group-item bg-transparent text-light p-1 text-nowrap" style="font-size: smaller;width: 160px;">NS-'.$row["id"].' '.$row["brand"].' '.$row["sku"].' SKU</li>';
    }
    // mysqli_close($con);
    return $list;
}
