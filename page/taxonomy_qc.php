...
function change_attribute_cate() {
    ...
    $.post("/get/get_taxonomy_cate_att_map.php", {
        ...
        function(data) {
            ...
            document.querySelectorAll(selector).forEach(el => el.style.display = 'none');
                ...
                document.getElementById("label_old_cate").style.display = "block";
...
function revised_record(sku) {
    ...
    for (let i = 0; i <= elements_no.length - 1; i++) {
        ...
        if (elements_no[i].checked) {
            ...
            $.post("/action/action_taxonomy_revised_record.php", {
...
function skip_taxonomy(sku){
    ...
    $.post("/action/action_taxonomy_skip.php", {
                    sku: sku
                },
                function(data)  {
                if (data.includes("error")) {
...
function summit_taxonomy(sku) {
    ...
    if (validate_checked == "ALL_CHECKED") {
        ...
        $.post("/action/action_taxonomy_update.php", {
                sku: sku,
                status: status,
