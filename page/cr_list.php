...
$bucket  = '<option value="all" selected class="nav-link text-start active"  data-bs-toggle="pill" data-bs-target="#v-pills-bucket" type="button" role="tab">
                <div class="row" >
...
while($row = mysqli_fetch_array($result)) {
    $bucket  .=
    '<option value="'.$row['prefix'].'" class="nav-link mt-0 mb-0text-start" data-bs-toggle="pill" data-bs-target="#v-pills-bucket" type="button" role="tab">
...
// echo $bucket;
echo'
<select id="bucket_selection" name="bucket_selection" class="border-0 bg-transparent" aria-label="Default select example" onchange="set_bucket();" >
'.$bucket.'
</select>
';
?>
                    ...
                    <div style="width:170px">
                        <div class="input-group input-group-sm" style="position: inherit;">
                            ...
                            <input type="number" class="form-control" style="position: inherit;"
                                id="page_navigator_input" min=1 value="1" onchange="getFilterInputValues('clear_param')">
                    ...
                    <div style="width:auto">
                    <button class="btn btn-danger btn-sm bg-gradient col " style="/* margin-left:10px; */margin: 2px 5px 0px 0px;/* height: 30px; */width: auto;position: absolute;right: 100px;padding-right: 20px;" type="button" data-bs-toggle="offcanvas" data-bs-target="#content_request_canvas" aria-controls="offcanvasExample">
                                <ion-icon size="small" name="add-outline" role="img" class="md icon-small hydrated" aria-label="add outline">
                                </ion-icon>
                                New Ticket
                            </button>
                        <ul class="nav nav-pills mb-3 row p-0 me-3 end-0" id="pills-tab"
            ...
            <div class="tab-pane fade show active" id="v-pills-bucket" role="tabpanel"
                ...
                <div id="bucket" class="m-3 mt-0">
                    <?php
                        // echo $_SERVER['DOCUMENT_ROOT']."/get/get_list_cr.php";
                        // include($_SERVER['DOCUMENT_ROOT']."/get/get_list_cr.php");
                    ?>
                </div>
            </div>
        </div>
...
function filter_backlogs(){
    ...
    el.slim.setSelected(['Pending', 'Inprogress', 'Waiting Execution', 'Waiting CTO','Waiting Buyer','Wait sync']);
...
function getFilterInputValues($clear_param) {
    ...
    if (view_mode == 'list') {
        var path = "/get/get_list_cr.php";
    } else if (view_mode == 'board') {
        var path = "/get/get_list_cr_board.php";
    }
    $.post(path, {
...
function set_bucket(bucket) {
    ...
    document.getElementById("bucket_selected").value = document.getElementById("bucket_selection").value;
    getFilterInputValues('clear_param');
    filter_backlogs();
}
getFilterInputValues('clear_param')
