...
while($row = mysqli_fetch_array($result)) {
  ...
  date_default_timezone_set("Asia/Bangkok");
  $con_project= mysqli_connect("service-gate-cds-omni-service-gate.a.aivencloud.com","avnadmin","AVNS_lAORtpjxYyc9Pvhm5O4","all_in_one_project","10628") or die("Error: " . mysqli_error($con));
...
<ul id="checklist_box" style="padding: 5px;">
    <?php
    // echo$_SERVER['DOCUMENT_ROOT'].'/get/get_checklist_cr.php?id='.$id.'&department='.$_SESSION["department"]
    include($_SERVER['DOCUMENT_ROOT'].'/get/get_checklist_cr.php?id='.$id.'&department='.$_SESSION["department"]); ?>
</ul>
<?php
    if($ticket_template<>"PJ"){
      echo '<hr>';
...
<ul id="checklist_box" style="padding: 5px;">
    <?php   include($_SERVER['DOCUMENT_ROOT'].'/get/get_checklist_cr.php?id='.$id.'&department='.$_SESSION["department"]); ?>
</ul>
<?php
    echo '
    <hr>
